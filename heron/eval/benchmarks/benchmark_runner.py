...
from vandl_adapter import OpenAIResponseGenerator

def load_questions(path):
    with open(path, "r") as file:
        return [json.loads(line) for line in file]

def get_evaluations(img_root, results, contexts, references, benchmark_config, verbose=True):
    def process_question(index, q, r):
        image_path = os.path.join(img_root, f"{q['image']}")
        question = q[benchmark_config['question_key']]
        answer = q["answer"]
        reference = r["answer"]
        context = contexts.loc[contexts["image"] == q['image'], benchmark_config['context_key']].values[0]
